...
tab1, tab2, tab3 = st.tabs(["📡 IVP AI Data Engine", "🧠 IVP AI Intelligence & Modeler", "🏢 Corporate Deal Ledger"])
...
# -------------------------------------------------------------------------------------
# TAB 2: IVP AI INTELLIGENCE & MODELER
# -------------------------------------------------------------------------------------
with tab2:
    ...
    with mc1:
        ...
        # User selects the deal date
        deal_date = st.date_input("Date of Deal Execution (Buy/Sell)", value=st.session_state.current_date, min_value=st.session_state.start_date, max_value=st.session_state.current_date)
        
        # Calculate exactly how many days are left in the month for hold duration bounding
        last_day_of_month = calendar.monthrange(deal_date.year, deal_date.month)[1]
        expiry_date = date(deal_date.year, deal_date.month, last_day_of_month)
        max_hold_days = (expiry_date - deal_date).days
        
        # If deal date is the last day, default to 1 day for math purposes
        if max_hold_days <= 0: max_hold_days = 1 
        
        # Fetch the exact spot price from history for the selected deal date
        deal_date_spot = st.session_state.history_df.loc[st.session_state.history_df['Date'] == deal_date, 'CDR_Index'].values[0]
        st.info(f"**CDR Spot Price on {deal_date.strftime('%B %d')}:** {deal_date_spot:.2f} mm")
        ...
        # Bound the slider strictly by the days left in the contract month
        s_days = st.slider("Hold Duration (Days to Expiry)", min_value=1, max_value=max_hold_days, value=max_hold_days)
        st.caption(f"*Contract settles on the last day of the month ({expiry_date.strftime('%B %d')}).*")
    ...
    with mc2:
        # IVP AI Math Generation using the specific deal date's entry value
        np.random.seed(42) 
        entry_val = deal_date_spot
        ...
        # Expected P&L at Expiry
        expected_pnl = (mean_final - entry_val) * s_lots * TICK_VALUE_RS if is_long else (entry_val - mean_final) * s_lots * TICK_VALUE_RS
        
        # Margin calculated accurately as 12% of the contract Notional Value
        notional_value = entry_val * s_lots * TICK_VALUE_RS
        req_margin = notional_value * MARGIN_INITIAL_PCT
        ...
        h2.markdown(f"<div class='hud-card'><div class='hud-title'>Expected Profit/Loss at Expiry</div><div class='hud-val' style='color: {'#166534' if expected_pnl>0 else '#991b1b'};'>₹{expected_pnl/CR_TO_RS:.2f} Cr</div></div>", unsafe_allow_html=True)
        h3.markdown(f"<div class='hud-card'><div class='hud-title'>Initial Margin Required (12%)</div><div class='hud-val'>₹{req_margin/CR_TO_RS:.2f} Cr</div></div>", unsafe_allow_html=True)
        
        st.caption("*Expected Profit/Loss represents the average probable outcome across 1,000 algorithmic weather simulations at the end of the holding period.*")
        st.write("")
